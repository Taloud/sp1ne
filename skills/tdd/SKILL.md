...
Before any planning, call the Skill tool with "project-docs" to detect the doc layout and load what that mode holds (glossary, lessons, ADRs, codemap).
...
If planning surfaces genuinely missing context (vague requirements, undefined terms), pause and suggest the user runs `/grill-with-docs` before continuing.
...
- Call the Skill tool with "check-conventions" to verify the diff against `CONTEXT.md` / ADRs / LESSONS before committing or opening a PR.
- If a generalisable rule emerged from the cycle (e.g. "every functional test rolls back", "this kind of mock always belongs at the boundary"), call the Skill tool with "lessons-add" to propose recording it.
