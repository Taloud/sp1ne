...
| **`/handoff`** | Write a portable markdown file and seed a session anywhere with it (or, in its second mode, launch a fresh background Claude session seeded with the summary via `claude --bg`). |
| **Subagent**   | Send the task to its own context window and get a report back (or, for a whole task, delegate it by model tier with `/orchestrator`). |
...
**1. Can you continue in this session?** Two things make the answer yes: the next phase needs this phase as a **primary source**, or you have enough usable context left (roughly 150k tokens) for the next phase to fit. Grilling → implementation is the standard yes: the implementation wants the reasoning verbatim, not a summary of it. Continue costs nothing and loses nothing, so rule it out before anything else.
...
That list is the whole clause. What `/handoff` buys is **portability**: a file that travels (or a fresh session that starts itself from it). If nothing is travelling, you don't need it.
