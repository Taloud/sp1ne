...
Keep steps 1–3 in **one unbroken context window**: don't compact or clear until after `/to-issues`, so the grilling, the PRD, and the issues all build on the same thinking.

At every phase boundary after that, walk the ordered tree in [PHASE-BOUNDARIES.md](PHASE-BOUNDARIES.md): first yes wins. `/compact` is the default, not the first reach.
...
- **`/handoff`**: when a session is full or you need to branch off, compact the conversation into a markdown doc, then open a **fresh session** and reference that file (or launch that fresh session itself, in the background, with `claude --bg`). The bridge between context windows. See [PHASE-BOUNDARIES.md](PHASE-BOUNDARIES.md) for when to reach for it over `/compact`: `/handoff` forks, `/compact` continues.
...
- **`/project-docs`**: the documentation layer itself: detects whether the project is `structured` / `domain` / `light` / `bootstrap`, and owns what to read, what may be written, and the ADR / CONTEXT / LESSONS formats. `/grill-with-docs`, `/check-conventions` and `/tdd` all go through it.
...
- **`/orchestrator`**: when a task is big and separable (many files, independent subtasks, a mechanical pass over the whole repo), it routes each subtask to the best-fit tier at or below the session model and keeps the session model as planner, router, and verifier. You can still type it to force it; it stays out of conversation-bound work (interviews, syntheses of the thread).
