...
For one-shot adds, call the Skill tool with "lessons-add" (it handles ID computation and section insertion). This file documents the format so a skill can read and update `LESSONS.md` inline when appropriate.
...
- **Separator**: new files use `### <ID>: <title>`. Older files separate the ID and the title with a dash; when appending to one, match the separator its entries already use.
