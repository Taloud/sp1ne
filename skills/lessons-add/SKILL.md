...
Variants: drop *Why* if obvious. Drop ` → <ref>` if none. Always keep the date. Existing files may separate the ID and the title with a dash instead of a colon: match whatever separator the existing entries already use, and reserve the colon form for a file with no entries yet.
